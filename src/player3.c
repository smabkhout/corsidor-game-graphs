...
struct move_t play(const struct move_t previous_move) {
 ...
 //   printf("%dcolorr222",move.c);
    ...
    vertex_t target = board->graph->objectives[best_order[index_objective]];
    ...
    if (move.m == board->graph->objectives[index_objective]) {
        ...
        if ((unsigned int)index_objective >= board->graph->num_objectives) {
            printf("🏁 All objectives visited. Returning to base!\n");
            board->graph->objectives[0] = board->graph->start[my_color];
            board->graph->num_objectives = 1;
            index_objective = 0;
        }
    }
    if (board != NULL) {
