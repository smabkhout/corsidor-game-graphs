...
/*void first_step(int argc, char **argv){
    ...
    for (int i = 1; i < argc && j < NUM_PLAYERS; i++) {
        if (argv[i][0] != '-' && argv[i][0]   != '1' && argv[i][0] != '2' &&
argv[i][0] != '0' && argv[i][0] != '5') { load_player(&players[j], argv[i]);
            j++;
        }
    }
}*/
 ...
 // board->graph = globalGraph;
  ...
  while (winner == -1 && turn_count < max_turns) {
    ...
    /* if (is_invalid(move,board)){
         printf("🤖 Player %s executed an illegal move. Did they even read the
    ...
    printf("Turn %d: Player %s plays %s to vertex %u\n", turn_count,
           players[current_player].get_player_name(),
           move_type_to_string(move.t), move.m);
   ...
   players[current_player].get_player_name()); winner = (current_player + 1) %
   NUM_PLAYERS; break;
    */
  }
