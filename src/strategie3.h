 ...
 * entre les sommets `v1` et `v2`. Retourne 1 si les sommets sont connectés, 0 sinon.
 */
...
/*
 * Renvoie l'indice du sommet non visité de distance minimale.
 * Utilisée par Dijkstra.
 */
int distance_minimal(int d[], int visited[], unsigned int n);

/*
 * Algorithme de Dijkstra adapté au jeu.
 * Calcule distances, prédécesseurs et successeur immédiat pour aller de a à b.
 */
...
/*
 * Calcule la matrice des distances entre chaque paire d'objectifs.
 */
...
/*
 * Calcule la distance totale parcourue selon une permutation d'objectifs.
 */
int calculate_total_dist(int n, int d[n][n], int tab[]);

/*
 * Génère la prochaine permutation lexicographique du tableau.
 * Retourne 1 si la permutation suivante a été générée, 0 si c'était la dernière.
 */
...
/*
 * Résout le problème du voyageur de commerce sur la liste d'objectifs non visités.
 * Remplit best_order avec l’ordre optimal et renvoie la distance minimale.
 */
int TSP(struct graph_t* graph, int best_order[], int obj_visited[], vertex_t pos_other_player);

/*
 * Trouve l'objectif le plus proche du joueur.
 * Retourne le sommet objectif le plus proche.
 */
 ...
 * Remplit out[] et retourne le nombre de voisins (≤ max_out).
 */
int num_neighbors(struct graph_t* graph, vertex_t v, vertex_t out[], int max_out);

/*
 * Essaie de poser un mur défensif entre pos_enemy→next_enemy et pos_enemy→nbr.
 * Si échec, renvoie fallback.
 */
