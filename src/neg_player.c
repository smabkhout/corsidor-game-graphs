...
struct move_t play(const struct move_t previous_move) {
  ...
  for (int i = 0; i < numberOfObjectives; ++i) {
    ...
    int obj_index_opp;
    for (int i = 0; i < numberOfObjectives; ++i) {
        if (!visited_objectives[i] && opp_distances_to_objectives[i] < min_distance_opp && opp_distances_to_objectives[i] != -1) {
            obj_index_opp = i;
    ...
    if (min_distance_opp < min_distance) {
        ...
        index_to_axial(opp_paths[obj_index_opp][1], board->graph->num_vertices, &l1, &c1);
