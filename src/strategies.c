...
struct game_state applyy_move(const struct game_state *state,
                              struct move_t move) {
...
int player_shortest_path_length(struct graph_t *g, struct player_tt *p,
...
color * evaluate(state, color); return (struct scored_move){ .score = score };
    ...
    int num_moves = availableMoves(legal_moves, state->graph,player1,
state->previous_moves[y].m);
...
{ if (depth == 0 || state->graph->num_edges == 0) { int score = color *
evaluate(state, color); return (struct scored_move){ .score = score };
    }
    ...
    struct move_t legal_moves[128];
    int num_moves = availableMoves(legal_moves, state->graph, player,
state->previous_moves[y].m);
...
struct scored_move negamax_ab(struct game_state *state, int depth, int alpha,
                              int beta, int color) {
  ...
  struct player_tt player = {.position = state->previous_moves[self].m,
                             ...
                             .c = self};
  int num_moves = availableMoves(legal_moves, state->graph, &player,
                                 state->previous_moves[opponent].m);
  ...
  for (int i = 0; i < num_moves; i++) {
    ...
    struct scored_move result =
        negamax_ab(&next, depth - 1, -beta, -alpha, -color);
    int score = -result.score;
...
struct scored_move negamax_naive(struct game_state *state, int depth,
                                 int color) {
  if (depth == 0 || is_game_over(state)) {
...
#define INF 1000000
struct scored_move minMaxi(struct game_state *state, int depth,
                           int maximizingPlayer, int originalMaxPlayer) {
  // Déterminer les identités des joueurs
  int current_player =
      maximizingPlayer ? originalMaxPlayer : (1 + originalMaxPlayer) % 2;
  ...
  if (depth == 0 || is_game_over(state)) {
    ...
    return (struct scored_move){
        .move = state->previous_moves[originalMaxPlayer], .score = score};
  }
  ...
  struct scored_move best_move = {.score =
...
void test_shortest_path_with_wall() {
  ...
  vertex_t *path = malloc(g->num_objectives * sizeof(vertex_t));
  int length = shortest_path_length(g, start, objective, 10, path);
...
void test_player_on_objective() {
  ...
  vertex_t *path = malloc(g->num_objectives * sizeof(vertex_t));
  int length = player_shortest_path_length(g, &p, objectives, 1, 10, path);
...
void test_multiple_objectives() {
  ...
  vertex_t *path = malloc(g->num_objectives * sizeof(vertex_t));
  int length = player_shortest_path_length(g, &p, objectives, 2, 10, path);
